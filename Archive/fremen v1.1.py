...
def strategy_5(prcSoFar, inst):
    model_tuple = ardl_models.get(inst)
    if model_tuple is None:
        return currentPos[inst]
    
    model, best_j = model_tuple

    current_price = prcSoFar[inst, -1]
    exog_series = prcSoFar[best_j, :]
    max_exog_lag = max(model.model.dl_lags[0]) + 1 if isinstance(model.model.dl_lags[0], (list, tuple)) else model.model.dl_lags[0] + 1

    # Use the correct number of most recent lags and reshape to (1, 1)
    x_latest = exog_series[-max_exog_lag:].reshape(1, -1)

    # Ensure we have exactly 1 column
    if x_latest.shape[1] != 1:
        x_latest = x_latest[:, -1].reshape(-1, 1)  # Take the last column only

    pred = model.forecast(steps=1, exog=x_latest)[0]
    expected_return = pred - current_price
    max_position = dlrPosLimit // current_price

    if expected_return > PRED_THRESHOLD:
        return max_position
    elif expected_return < -PRED_THRESHOLD:
        return -max_position
    else:
        return currentPos[inst]
    ...
    for i in range(nInst):
        ...
        for j in range(nInst):
            ...
            if corr > best_corr:
                ...
                best_j = j
        if best_j is not None:
            x = prc_window[best_j].reshape(-1, 1)
            y = prc_window[i]
            x_series = prc_window[best_j].reshape(-1, 1)
            model = ARDL(endog=y, lags=5, exog=x.reshape(-1, 1), order=5).fit()
            ardl_models[i] = (model, best_j)
